<div x-data="fileUpload()" x-on:drop.prevent="handleFileDrop($event)" x-on:dragover.prevent="isDropping = true"
        ...
        <div class="extraOutline p-4 bg-white w-max m-auto rounded-lg  bg-gray-400">
            ...
            <div class="grid grid-cols-2 gap-4">
                @if(auth()->user()->role==\App\Models\User::ROLE_ADMIN)
                    <div>
                        ...
                        <select wire:model.live="post_office_box_id" class="mt-1 p-2 w-full border rounded-md">
                            ...
                            @foreach($pobs as $key => $pob)
                                <option value="{{$key}}">{{$pob}}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                <div>
                    ...
                    <input type="text" wire:model.live="page_number" class="mt-1 p-2 w-full border rounded-md"
                           placeholder="Input Page Number">
                </div>
                @if(auth()->user()->role==\App\Models\User::ROLE_ADMIN)
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">Amount</label>
                        <input type="text" wire:model.live="amount" class="mt-1 p-2 w-full border rounded-md"
        ...
        <div class="">
            @if ($uploaded_file)
                <div class="p-4 border rounded mt-2  bg-gray-400">
                    ...
                    <button wire:click.prevent="removeUploadedFile" class="mt-2 bg-red-500 text-white p-2 rounded">
                        Remove
                    </button>
                </div>
            @endif
                        ...
                        <div class="p-4 border rounded mt-2 bg-gray-400 w-full flex flex-row justify-between">
                            ...
                            <div class="float-end flex flex-col">
                                ...
                                <a href="#" wire:click="viewActivity({{$file->id}})"
                                   ...
                                   x-data="" x-on:click.prevent="$dispatch('open-modal', 'view-activity')">View
                                    Activity</a>
                            </div>
                        </div>
                    @endif
